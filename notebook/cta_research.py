"""
cta_research.py — Multi-instrument CTA strategy research.
...
Strategy:
  ...
  - Universe: CSI 300, GEB (创业板), Shenzhen Composite

Note: Index data only supports long-only. For true long/short CTA,
      use futures data and set allow_short=True.
"""
...
INSTRUMENTS = {
    'CSI300': '000300.XSHG.csv',   # 沪深300
    'GEB':    '399006.XSHE.csv',   # 创业板指
    'SZCOMP': '399324.XSHE.csv',   # 深证综指
}
...
class CTAStrategy(bt.Strategy):
    ...
    params = dict(
        ...
        atr_mult=2.0,
        adx_threshold=20,
        risk_pct=0.02,       # fraction of portfolio risked per instrument
        allow_short=False,   # requires futures data
    )
    ...
    def __init__(self):
        ...
        self.pending = {}  # name → order (to avoid double orders)
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            ...
            self.log(f'{name} {action} size={order.executed.size:.0f} '
        ...
        for d in self.datas:
            ...
            # ── long ──────────────────────────────────────────────────────────
                ...
                if ind['stop'] is None:
                    ...
                    ind['stop'] = max(ind['stop'], new_stop)  # ratchet up
            ...
            # ── short ─────────────────────────────────────────────────────────
                ...
                if ind['stop'] is None:
                    ...
                    ind['stop'] = min(ind['stop'], new_stop)  # ratchet down
...
def plot_equity(strat, title='CTA Portfolio Equity Curve'):
    ...
    plt.tight_layout()
    plt.savefig('cta_equity.png', dpi=150, bbox_inches='tight')
    print('Equity curve saved → cta_equity.png')
    plt.show()
...
sharpe_df = pd.DataFrame(
    sharpe_matrix,
    index=[f'slow={s}' for s in SLOW_RANGE],
    columns=[f'mult={m}' for m in MULT_RANGE],
)
...
ax.set_title('Sharpe Ratio Heatmap (slow × atr_mult)')
...
plt.tight_layout()
plt.savefig('cta_sharpe_heatmap.png', dpi=150, bbox_inches='tight')
print('\nSharpe heatmap saved → cta_sharpe_heatmap.png')
plt.show()
